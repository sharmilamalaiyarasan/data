...
files = glob.glob("data/demographic/*.csv")
all_chunks = []
...
demo_district = pd.concat(all_chunks, ignore_index=True)

demo_district = demo_district.groupby(
    ["date", "state", "district"], as_index=False
).sum()

demo_district["total_demographic_updates"] = (
    demo_district["demo_age_5_17"] + demo_district["demo_age_17_"]
)

demo_district.to_csv("output/demographic_district.csv", index=False)
print("✅ Demographic processing completed successfully!")
