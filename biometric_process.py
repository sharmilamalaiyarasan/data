...
def correct_district_name(state, district, threshold=90):
    ...
    match = process.extractOne(district, candidates, scorer=fuzz.token_sort_ratio)
    return match[0] if match and match[1] >= threshold else None

files = glob.glob("data/biometric/*.csv")
all_chunks = []
...
for file in files:
    ...
    for chunk in pd.read_csv(file, chunksize=200_000):
        ...
        )[["bio_age_5_17", "bio_age_17_"]].sum()

        all_chunks.append(agg)

bio_district = pd.concat(all_chunks, ignore_index=True)

bio_district = bio_district.groupby(
    ["date", "state", "district"], as_index=False
).sum()

bio_district["total_biometric_updates"] = (
    bio_district["bio_age_5_17"] + bio_district["bio_age_17_"]
)

bio_district.to_csv("output/biometric_district.csv", index=False)
print("✅ Biometric processing completed successfully!")
